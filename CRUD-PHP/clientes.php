...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if ($action === 'consult') {
        ...
        // Insertar datos
        $stmt = $conn->prepare("INSERT INTO clientes (cedula, nombres, apellidos, direccion, telefono, email, contraseña) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($cedula !== $old_cedula) {
            $stmt = $conn->prepare("UPDATE clientes SET cedula=?, nombres=?, apellidos=?, direccion=?, telefono=?, email=?, contraseña=? WHERE cedula=?");
            $stmt->bind_param("ssssssss", $cedula, $nombres, $apellidos, $direccion, $telefono, $email, $contraseña, $old_cedula);
        } else {
            $stmt = $conn->prepare("UPDATE clientes SET nombres=?, apellidos=?, direccion=?, telefono=?, email=?, contraseña=? WHERE cedula=?");
...
<body>
    <div class="container">
        <h1 class="title">Datos De Clientes</h1>
        <div class="input">
